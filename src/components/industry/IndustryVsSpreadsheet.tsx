...
type Props = {
  title?: string;
  /** Intro sous le H2 — défaut propreté (pages solutions historiques). */
  intro?: {
    href: string;
    linkLabel: string;
    after: string;
...
const defaultIntro = {
  ...
  linkLabel: "logiciel pour entreprise de nettoyage",
  after: `est pensé pour la propreté en France — prix public, modules métier, support local au ${site.phone}.`,
};

export function IndustryVsSpreadsheet({ title, intro = defaultIntro }: Props) {
  return (
      ...
      <div className="container">
        <Reveal>
          ...
          <p className="mt-4 max-w-xl text-slate">
            Notre{" "}
            <Link href={intro.href} className="link-accent font-semibold">
              {intro.linkLabel}
            </Link>{" "}
            {intro.after}
          </p>
