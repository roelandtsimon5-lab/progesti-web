/* eslint-disable */
/**
 * Builder pages EV — copy longue + démo unique par page.
 * Ne pas fusionner avec sécurité / propreté.
 */
...
export function buildEvEntry(raw: EvPageSpec): MarketingPageEntry {
  ...
  const isPilier = spec.key === "logiciel-espace-vert";
  const breadcrumbs =
    isPilier
      ? [
          { label: "Accueil", href: "/" },
      ...
      : isHub
        ? [
            { label: "Accueil", href: "/" },
            { label: "Espaces verts", href: "/espace-vert" },
          ]
        : [
            { label: "Accueil", href: "/" },
            { label: "Espaces verts", href: "/espace-vert" },
  ...
  const config: IndustryPageConfig = mergeIndustryConfig(defaultIndustryConfig, {
    slug: isHub ? "espace-vert" : spec.key,
    seo: {
      title: spec.seoTitle,
    ...
    hero: {
      ...
      trustLeft: spec.trustLeft ?? "Verticale espaces verts · terrain & bureau",
      productStripLabel: spec.productStrip,
      mediaPhoto: "/hero-planning.webp",
      mediaPhotoAlt: `${spec.h1Lead} — planning espaces verts PROGESTI`,
    },
    empathy: {
      h2: spec.empathyH2,
    ...
    showcase: {
      ...
      imageAlt: `${spec.showcaseTitle} — PROGESTI espaces verts`,
